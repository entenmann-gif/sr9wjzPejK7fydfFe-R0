...
loginForm?.addEventListener('submit', async (event) => {
  ...
  if (!window.accountStore.enabled) {
    ...
    if (reason === 'missing_config') {
      const debug = window.accountStore.configDebug ?? {};
      const missingParts = [
        !debug.hasUrl ? 'SUPABASE_URL' : null,
        !debug.hasAnonKey ? 'SUPABASE_ANON_KEY' : null,
      ].filter(Boolean).join(' und ');

      loginMessage.textContent = `Supabase-Konfiguration fehlt: ${missingParts || 'Werte nicht erkennbar'}. Bitte \`supabase-config.js\` prüfen.`;
      ...
      loginMessage.textContent = 'Supabase-Initialisierung ist abgestürzt. Bitte Browser-Konsole öffnen.';
    } else {
